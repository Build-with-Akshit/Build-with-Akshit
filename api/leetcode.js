...
function getUTCDayNum(ts) {
  const sec = parseInt(ts, 10);
  const timestampSec = sec > 1e11 ? Math.floor(sec / 1000) : sec;
  return Math.floor(timestampSec / 86400);
}
...
function formatDate(d, includeYear = false) {
  ...
  return includeYear ? `${m} ${day}, ${yr}` : `${m} ${day}`;
}

function formatRange(startDate, endDate, isTotal = false) {
  if (!startDate) return "N/A";
  const nowYear = new Date().getUTCFullYear();
  const sYr = startDate.getUTCFullYear();
  
  if (isTotal) {
    const sStr = formatDate(startDate, sYr !== nowYear);
    return `${sStr} - Present`;
  }
  
  if (!endDate) return formatDate(startDate, sYr !== nowYear);

  const eYr = endDate.getUTCFullYear();
  if (startDate.getTime() === endDate.getTime()) {
    return formatDate(startDate, sYr !== nowYear);
  }

  if (sYr === eYr) {
    const sStr = formatDate(startDate, false);
    const eStr = formatDate(endDate, sYr !== nowYear);
    return `${sStr} - ${eStr}`;
  } else {
    const sStr = formatDate(startDate, true);
    const eStr = formatDate(endDate, true);
    return `${sStr} - ${eStr}`;
  }
}

function parseSubmissionStats(submissionCalendar, apiHighestStreak = 0) {
  let calMap = {};
  if (typeof submissionCalendar === "string") {
    ...
    calMap = submissionCalendar;
  }

  const activeDaysSet = new Set();
  for (const tsStr of Object.keys(calMap)) {
    if (calMap[tsStr] > 0) {
      const dayNum = getUTCDayNum(tsStr);
  ...
  if (sortedDays.length === 0) {
    return {
      totalSubPeriod: "N/A",
      currentStreak: 0,
      currentStreakPeriod: "N/A",
      highestStreak: apiHighestStreak || 0,
      highestStreakPeriod: "N/A",
    };
  }

  const firstDay = sortedDays[0];
  const firstDate = new Date(firstDay * 86400 * 1000);
  const totalSubPeriod = formatRange(firstDate, null, true);

  const segments = [];
  let currentSeg = { start: sortedDays[0], end: sortedDays[0], length: 1 };

  for (let i = 1; i < sortedDays.length; i++) {
    if (sortedDays[i] === currentSeg.end + 1) {
      currentSeg.end = sortedDays[i];
      currentSeg.length++;
    } else {
      segments.push(currentSeg);
      currentSeg = { start: sortedDays[i], end: sortedDays[i], length: 1 };
    }
  }
  segments.push(currentSeg);

  const nowSec = Math.floor(Date.now() / 1000);
  const todayDay = getUTCDayNum(nowSec);
  const yesterdayDay = todayDay - 1;

  let activeCurrentSeg = null;
  const lastSeg = segments[segments.length - 1];

  if (lastSeg.end === todayDay || lastSeg.end === yesterdayDay) {
    activeCurrentSeg = lastSeg;
  }

  let currentStreakVal = 0;
  let currentStreakPeriod = "N/A";
  if (activeCurrentSeg) {
    currentStreakVal = activeCurrentSeg.length;
    const sDate = new Date(activeCurrentSeg.start * 86400 * 1000);
    const eDate = new Date(activeCurrentSeg.end * 86400 * 1000);
    currentStreakPeriod = formatRange(sDate, eDate, false);
  }

  let bestSeg = segments[0];
  for (const seg of segments) {
    if (seg.length >= bestSeg.length) {
      bestSeg = seg;
    }
  }

  const highestStreakVal = Math.max(apiHighestStreak, bestSeg ? bestSeg.length : 0);
  let highestStreakPeriod = "N/A";

  if (bestSeg) {
    const sDate = new Date(bestSeg.start * 86400 * 1000);
    const eDate = new Date(bestSeg.end * 86400 * 1000);
    highestStreakPeriod = formatRange(sDate, eDate, false);
  }
...
async function renderLeetCodeCard(data, options = {}) {
  ...
  const calendarStats = parseSubmissionStats(
    user.userCalendar?.submissionCalendar,
    user.userCalendar?.streak || 0
  );

  const totalSubPeriod = calendarStats.totalSubPeriod;
  const currentStreak = calendarStats.currentStreak;
  const currentStreakPeriod = calendarStats.currentStreakPeriod;
  const highestStreak = calendarStats.highestStreak;
  const highestStreakPeriod = calendarStats.highestStreakPeriod;
...
<defs>
  ...
  <!-- LeetCode Left Bracket Clip -->
  <clipPath id="lc-left-bracket">
    <polygon points="-2,-2 11.2,-2 8,24 -2,24" />
  </clipPath>
</defs>
...
<g transform="translate(${pad}, 38)">
  ...
  <g transform="translate(-2, -18) scale(1.15)">
    <!-- Orange Base (Top-right arm & bottom-right loop) -->
    ...
    <!-- White Left Angle Bracket -->
    <path fill="#ffffff" clip-path="url(#lc-left-bracket)" d="M13.483 0a1.374 1.374 0 0 0-.961.438L7.116 6.226l-3.854 4.126a5.266 5.266 0 0 0-1.209 2.104 5.35 5.35 0 0 0-.125.513 5.527 5.527 0 0 0 .062 2.362 5.83 5.83 0 0 0 .349 1.017 5.938 5.938 0 0 0 1.271 1.818l4.277 4.193.039.038c2.248 2.165 5.852 2.133 8.063-.074l2.396-2.392c.54-.54.54-1.414.003-1.955a1.378 1.378 0 0 0-1.951-.003l-2.396 2.392a3.021 3.021 0 0 1-4.205.038l-.02-.019-4.276-4.193c-.652-.64-.972-1.469-.948-2.263a2.68 2.68 0 0 1 .066-.523 2.545 2.545 0 0 1 .619-1.164L9.13 8.114c1.058-1.134 3.204-1.27 4.43-.278l3.501 2.831c.593.48 1.461.387 1.94-.207a1.384 1.384 0 0 0-.207-1.943l-3.5-2.831c-.8-.647-1.766-1.045-2.774-1.202l2.015-2.158A1.384 1.384 0 0 0 13.483 0z"/>
    <!-- Middle Light Gray Horizontal Bar -->
    <path fill="#b3b3b3" d="M10.617 12.815a1.38 1.38 0 0 0-1.38 1.382 1.38 1.38 0 0 0 1.38 1.382H20.79a1.38 1.38 0 0 0 1.38-1.382 1.38 1.38 0 0 0-1.38-1.382z"/>
  </g>
...
<g transform="translate(500, 115)">
  <text x="0" y="2" text-anchor="middle" font-size="32" font-weight="800" fill="${white}">${totalSub}</text>
  <text x="0" y="30" text-anchor="middle" font-size="12" font-weight="500" fill="${grey}">Total Submissions</text>
  <text x="0" y="52" text-anchor="middle" font-size="11" font-weight="500" fill="${green}">${totalSubPeriod}</text>
</g>
...
<g transform="translate(660, 120)">
  ...
  <text x="0" y="8" text-anchor="middle" font-size="28" font-weight="800" fill="${white}">${currentStreak}</text>
  <text x="0" y="54" text-anchor="middle" font-size="13" font-weight="700" fill="${white}">Current Streak</text>
  <text x="0" y="74" text-anchor="middle" font-size="11" font-weight="500" fill="${green}">${currentStreakPeriod}</text>
</g>

<!-- Highest Streak -->
<g transform="translate(820, 115)">
  <text x="0" y="2" text-anchor="middle" font-size="32" font-weight="800" fill="${white}">${highestStreak}</text>
  <text x="0" y="30" text-anchor="middle" font-size="12" font-weight="500" fill="${grey}">Highest Streak</text>
  <text x="0" y="52" text-anchor="middle" font-size="11" font-weight="500" fill="${green}">${highestStreakPeriod}</text>
</g>
